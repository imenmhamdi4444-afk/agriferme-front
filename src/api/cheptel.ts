...
export const getAnimauxMalades = async () => {
  ...
  const malades = all.filter((a: any) =>
    (a.etat_sante || a.etatSante) === 'Malade' || (a.etat_sante || a.etatSante) === 'Critique'
  ).map(mapCheptel);
...
export const deleteCheptel = async (id: number) => {
  const res = await fetch(`${API_URL}/cheptels/${id}`, { method: 'DELETE' });
  return { data: await res.json(), status: res.status };
};
...
export const prendreRdv = async (rdvData: any) => {
  ...
  const res = await fetch(`${API_URL}/cheptels/${id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ etatSante: 'Critique' }),
  });
  return { data: await res.json(), status: res.status };
};